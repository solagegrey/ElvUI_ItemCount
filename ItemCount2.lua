...
IC.version = GetAddOnMetadata("ElvUI_ItemCount2", "Version")

--------------- VARIABLES ------------------

local menuFrame = CreateFrame("Frame", "ItemCount2Menu", E.UIParent, "UIDropDownMenuTemplate")
...
-- find current QoH; chime or sound bell if appropriate
	...
	local NewQuantity
	if cObj.QoH == nil then cObj.QoH = 0; end
...
local function OnEvent(self, event, ...)
	...
	ButtonText = "Select an Item to Watch"
...
local function doToolTip(cObj)
	...
	local qoh_color = C_RED
	local item_color = C_WHITE
	local ifWatched = ""
	...
	if cObj.QoH >= cObj.Goal and cObj.Goal > 0 then
		qoh_color = C_GREEN
	else
		qoh_color = C_WHITE
	end
	if cObj.Frozen == true then item_color = C_AQUA; end
	if cObj.Chime == true then chimestr = "Chime "..C_GREEN.."ON"; end
	if cObj.Silent == true then silentstr = "Silent "..C_GREEN.."ON"; end
	if pf.watched == cObj.index then ifWatched = C_AQUA.." *W*"; end

	DT.tooltip:AddDoubleLine(item_color..cObj.item..ifWatched, 
		C_YELLOW.."QoH "..qoh_color..string.format("%.0f", cObj.QoH), 1, 1, 1, 0.8, 0.8, 0.8)
...
-- Show DataText Dropdown
	...
	DT.tooltip:AddLine(" ")
	DT.tooltip:AddLine("ITEM: "..C_WHITE.."normal "..C_AQUA.."frozen")
	DT.tooltip:AddLine("QoH:  "..C_RED.."goal pending "..C_GREEN.."goal met")
...
function InitDB()
	...
	db = ADB:New("ItemCount2DB", defaults, true)
...
function IC:ContainerFrameItemButton_OnModifiedClick(...)
	...
	if select(2,...) == "RightButton" and IsAltKeyDown() and not IsControlKeyDown() and not IsShiftKeyDown() and not CursorHasItem() then
		...
		elseif Count5.frozen == false then setItem(Count5, newItem)
			...
			print("Item Count ERROR: either there are no unfrozen Count slots")
		end
...
function SetDefaults(obj)
   ...
   shallowcopy(obj.c1, defaults.c1)
   shallowcopy(obj.c2, defaults.c2)
   shallowcopy(obj.d3, defaults.c3)
   shallowcopy(obj.c4, defaults.c4)
   shallowcopy(obj.c5, defaults.c5)
end
...
--EP:RegisterPlugin('ItemCount2', InjectOptions, false, Version)
EP:RegisterPlugin(..., InjectOptions)
...
DT:RegisterDatatext('ItemCount2', 'Miscellaneous', {"PLAYER_ENTERING_WORLD","BAG_UPDATE_DELAYED"}, OnEvent, OnUpdate, Click, OnEnter, OnLeave, 'Item Count', nil, ValueColorUpdate)
